...
import com.dscfgos.utils.connection.ConnectionManager;
...
public class BaseWrapperFactory<T> 
{
	...
	// ****************************************************************************************
	private static String SQL_SELECT_ITEM 		= "SELECT * FROM {0}{1} WHERE {2};";
	private static String SQL_SELECT_ALL_ITEMS 	= "SELECT * FROM {0}{1} ;";
	private String currentSchema = "";
	public String getCurrentSchema() 
	{
		return (this.currentSchema != null && this.currentSchema.length() > 0)?this.currentSchema+".":"";
	}
	// ****************************************************************************************
	...
	// ****************************************************************************************
	public BaseWrapperFactory(String currentSchema) 
	{
	...
	public T getItemByFields(Class<T> type, FieldValue[] sqlFields, WhereOperation operation)
	{
		...
		finally 
		{
			...
			{
				Logger lgr = Logger.getLogger(BaseWrapperFactory.class.getName());
	...
	private ResultSet getItemResultSet(FieldValue[] sqlFields, Class<T> type, WhereOperation operation)
	{
		...
		if(sqlFields != null && sqlFields.length > 0)
		{
			...
			try 
			{
				...
				{
					query = StringUtils.substitute(SQL_SELECT_ITEM,this.getCurrentSchema(), type.getSimpleName(), this.getWhereANDClause(sqlFields));	
				}
				else
				{
					query = StringUtils.substitute(SQL_SELECT_ITEM,this.getCurrentSchema(), type.getSimpleName(), this.getWhereORClause(sqlFields));
				}
	...
	private ResultSet getAllItemsResultSet(Class<T> type)
	{
		...
		try 
		{
			String query = StringUtils.substitute(SQL_SELECT_ALL_ITEMS, this.getCurrentSchema(),type.getSimpleName());
	...
	private String getInsertSQLString(Class<T> type, Object item)
	{
		...
		if(item != null)
		{
			...
			result = StringUtils.substitute("INSERT INTO {0}{1} ({2}) VALUES ({3});", this.getCurrentSchema(),type.getSimpleName(),sqlFieldsString, sqlValuesString);
		}
	...
	private String getUpdateSQLString(Class<T> type, FieldValue[] toUpdateSqlFields, FieldValue[] sqlFields)
	{
		...
		if(toUpdateSqlFields != null && toUpdateSqlFields.length > 0)
		{
			...
			result = StringUtils.substitute("UPDATE {0}{1} SET {2} WHERE {3};", this.getCurrentSchema(),type.getSimpleName(),sqlFieldsString, where);
		}
